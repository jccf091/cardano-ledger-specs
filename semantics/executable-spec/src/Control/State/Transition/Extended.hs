...
applySTSInternal ::
  ...
  RuleContext rtype s ->
  m (State s, [[PredicateFailure s]])
applySTSInternal _ap goRule ctx =
  ...
  where
    ...
    applySTSInternal' STransition jc =
      goRule jc `traverse` transitionRules
